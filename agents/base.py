...
class BaseAgent(ABC):
    ...
    def apply_safety_constraints(self, action: np.ndarray, observation: np.ndarray) -> np.ndarray:
        """
    ...
    def update_performance_metrics(self, reward: float, glucose: float, action: np.ndarray):
    ...
    def evaluate_policy(
        self, env, n_episodes: int = 10, deterministic: bool = True
    ) -> Dict[str, float]:
        """
        ...
        eval_metrics = {
            ...
            "glucose_cv": (
                np.std(glucose_values) / np.mean(glucose_values) if glucose_values else 0.0
            ),
            "hypoglycemia_rate": (
                np.mean([g < 70 for g in glucose_values]) if glucose_values else 0.0
            ),
            "hyperglycemia_rate": (
                np.mean([g > 180 for g in glucose_values]) if glucose_values else 0.0
            ),
        }

        logger.info(
            f"Evaluation completed: TIR={eval_metrics['time_in_range']:.1%}, "
