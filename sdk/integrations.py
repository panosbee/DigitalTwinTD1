...
class TandemTslim(InsulinPump):
    ...
    async def get_reading(self) -> DeviceReading:
        return DeviceReading(
            ...
            raw_data={
                "basal_rate": self.basal_rate,
                "control_iq_active": self.control_iq_enabled
            }
        )
...
def get_device_info(device_type: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "category": "CGM" if issubclass(device_class, CGMDevice) else 
                   "Pump" if issubclass(device_class, InsulinPump) else
                   "Wearable" if issubclass(device_class, SmartWatch) else "Other",
