...
class UVAPadovaModel(BaseModel):
    ...
    def _glucose_insulin_model(
        self, state: np.ndarray, t: float, meal_input: float, insulin_input: float
    ) -> np.ndarray:
        """
        ...
        EGP = max(
            0,
            self.params["EGP0"] * (1 - self.params["kp1"] * X)
            - self.params["kp2"] * G
            - self.params["kp3"] * I,
        )
        ...
        return np.array(
            [dGp_dt, dGt_dt, dI_dt, dX_dt, dQsto1_dt, dQsto2_dt, dQgut_dt, dIsc1_dt, dIsc2_dt]
        )
    ...
    def predict(self, X: pd.DataFrame, horizon: int = 30, **kwargs) -> np.ndarray:
        ...
        simulation_results = self.simulate(
            duration=horizon, meals=meals, insulin=insulin, initial_glucose=initial_glucose
        )
    ...
    def get_simulation_details(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "simulation_step": self.simulation_step,
            "initial_state": (
                self.initial_state.tolist() if self.initial_state is not None else None
            ),
        }
