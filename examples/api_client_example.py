...
class DigitalTwinAPIClient:
    ...
    def connect_device(self, device_type, patient_id=None):
        """Σύνδεση συσκευής."""
        data = {"device_type": device_type, "patient_id": patient_id}
        response = self.session.post(f"{self.base_url}/device/connect", json=data)
        return response.json()

    def predict_glucose(self, horizon_minutes=30, patient_id=None):
        """Πρόβλεψη γλυκόζης."""
        data = {"horizon_minutes": horizon_minutes, "patient_id": patient_id}
        response = self.session.post(f"{self.base_url}/predict/glucose", json=data)
        return response.json()
...
def demo_real_time_monitoring():
    ...
    for i in range(5):
        ...
        risk_emoji = {"low": "✅", "medium": "⚠️", "high": "🚨"}.get(pred["risk_level"], "❓")

        print(
            f"   {risk_emoji} Glucose: {pred['current_glucose']:.0f} → {pred['predicted_glucose']:.0f} mg/dL"
        )
