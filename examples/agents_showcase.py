...
class AgentsShowcase:
    ...
    async def _demo_single_agent_training(self):
        ...
        for algo in algorithms:
            ...
            try:
                ...
                )  # Ensure training_metrics is handled if it was F841
    ...
    async def _demo_multi_agent_ensemble(self):
        ...
        try:
            ...
            if MultiAgentCoordinator is not None:
                coordinator = MultiAgentCoordinator(agents=agents_list, voting_strategy="weighted")
            ...
            for i, (obs, scenario) in enumerate(zip(test_observations, scenarios)):
                ...
                for agent_name, agent in self.trained_agents.items():
                    try:
                        ...
                        print(
                            f"     • {agent_name}: {action[0] if hasattr(action, '__getitem__') else action:.2f} units"
                        )
                    except Exception as e:
                ...
                try:
                    ...
                    action_val = (
                        ensemble_action[0]
                        if hasattr(ensemble_action, "__getitem__")
                        else ensemble_action
                    )
        ...
        try:
            ...
            for scenario in safety_scenarios:
                ...
                if self.trained_agents:
                    ...
                    for agent_name, agent in self.trained_agents.items():
                        try:
                            raw_action = agent.model.predict(
                                scenario["observation"], deterministic=True
                            )[0]
                            safe_action = agent.apply_safety_constraints(
                                raw_action, scenario["observation"]
                            )
                            ...
                            print(f"     • {agent_name}:")
                            raw_action_val = (
                                raw_action[0] if hasattr(raw_action, "__getitem__") else raw_action
                            )
                            safe_action_val = (
                                safe_action[0]
                                if hasattr(safe_action, "__getitem__")
                                else safe_action
                            )
            ...
            if self.trained_agents:
                for agent_name, agent in self.trained_agents.items():
                    ...
                    print(
                        f"   • {agent_name}: {violations} violations, Safety Score: {safety_score:.2f}"
                    )
            else:
    ...
    async def _demo_real_time_deployment(self):  # noqa: C901
        ...
        try:
            ...
            for step in range(time_steps):
                ...
                obs = np.array(
                    [current_glucose / 100, 0.0, hour_of_day / 24, 0.0, 0.0, 0.5, 0.8, 0.5]  # Trend
                )
                ...
                if step % 72 == 0:  # Every 6 hours
                    print(
                        f"   ⏰ Hour {step//12:2d}: Glucose={current_glucose:5.1f} mg/dL, "
    ...
    def _simulate_glucose_dynamics(self, glucose, insulin, hour):
        ...
        total_change = (
            insulin_effect * glucose_factor + dawn_effect + meal_effect + random_effect
        ) * 0.1  # Scale για 5-min
    ...
    async def _demo_performance_comparison(self):
        ...
        try:
            ...
            print(
                f"   • Time in Range: {self.performance_results[best_tir]['time_in_range']*100:.1f}%"
            )
    ...
    async def _create_agents_dashboard(self):
        ...
        try:
            ...
            fig.suptitle(
                "🤖 Intelligent Agents Dashboard - Digital Twin T1D", fontsize=16, fontweight="bold"
            )
