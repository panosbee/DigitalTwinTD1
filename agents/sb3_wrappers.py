                ...
                try:
                    ...
                    if avg_safety < self.safety_threshold:
                        self.safety_violations.append(
                            {"step": self.num_timesteps, "safety_score": avg_safety}
                        )

                        if self.verbose > 0:
                            logger.warning(
                                f"Safety violation at step {self.num_timesteps}: "
...
if SB3_AVAILABLE:
    ...
    class PPOAgent(SB3Agent):
        ...
        def __init__(self, env, config: AgentConfig, **kwargs):
            ...
            super().__init__(
                algorithm="PPO", env=env, config=config, policy_kwargs=ppo_policy_kwargs
            )
    ...
    class SACAgent(SB3Agent):
        ...
        def __init__(self, env, config: AgentConfig, **kwargs):
            ...
            super().__init__(
                algorithm="SAC", env=env, config=config, policy_kwargs=sac_policy_kwargs
            )
    ...
    class TD3Agent(SB3Agent):
        ...
        def __init__(self, env, config: AgentConfig, **kwargs):
            ...
            super().__init__(
                algorithm="TD3", env=env, config=config, policy_kwargs=td3_policy_kwargs
            )
...
def make_sb3(
    algorithm: str, env, config: Optional[AgentConfig] = None, safety_layer: bool = True, **kwargs
) -> SB3Agent:
    """
...
def create_vectorized_env(env_fn, n_envs: int = 4, seed: int = 42, use_subprocess: bool = True):
    """
...
def train_ppo_agent(
    env, total_timesteps: int = 1000000, save_path: Optional[str] = None, **kwargs
) -> PPOAgent:
