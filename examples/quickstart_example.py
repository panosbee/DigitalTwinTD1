        ...
        for meal_hour in [8, 13, 19]:
            ...
            if meal_idx < len(time_points):
                ...
                if peak_time < len(glucose_with_meals):
                    ...
                    meal_effect = 80 * np.exp(
                        -((np.arange(len(glucose_with_meals)) - peak_time) ** 2) / (2 * 20**2)
                    )
                    ...
                    insulin_effect = -60 * np.exp(
                        -((np.arange(len(glucose_with_meals)) - meal_idx - 12) ** 2) / (2 * 15**2)
                    )
...
def main():
    ...
    try:
        transformer_model = TransformerModel(
            sequence_length=12, d_model=64, num_layers=2, epochs=15, patience=5, hybrid=False
        )
    ...
    try:
        ...
        mechanistic_pred = mechanistic_model.predict(
            X_test, horizon=30, initial_glucose=y_test.iloc[0]
        )
