...
class VectorMemoryStore:
    ...
    def add_embedding(self, embedding: np.ndarray, metadata: Optional[Dict] = None):
        ...
        if embedding.shape[0] != self.embedding_dim:
            if embedding.shape[0] == 1 and embedding.shape[1] == self.embedding_dim: # (1, D)
                ...
                raise ValueError(f"Embedding dimension mismatch. Expected {self.embedding_dim}, got {embedding.shape}")
    ...
    def search_similar(self, query_embedding: np.ndarray, k: int = 5) -> List[Tuple[np.ndarray, Dict, float]]:
        """
        ...
        distances, indices = self.index.kneighbors(query_embedding, n_neighbors=min(k, self.embeddings.shape[0]))
