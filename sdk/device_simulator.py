...
class VirtualPatient:
    ...
    def __init__(self, 
                 ...
                 carb_ratio: float = 10):  # grams per unit
    ...
    def update_glucose(self, minutes_elapsed: float = 5) -> float:
        ...
        glucose_change = (liver_glucose + carb_effect + stress_effect + dawn_effect 
                         - insulin_effect - activity_effect)
...
class DeviceSimulationPlatform:
    ...
    def create_scenario(self, scenario_name: str) -> Tuple[VirtualPatient, VirtualCGM, VirtualInsulinPump]:
        ...
        patient = VirtualPatient(
            patient_id=f"scenario_{scenario_name}",
            **config
        )
...
def create_virtual_device(device_type: str, device_id: str = None) -> Union[VirtualCGM, VirtualInsulinPump]:
    """Factory για virtual devices."""
    patient = VirtualPatient()
    
    if "cgm" in device_type.lower() or "dexcom" in device_type.lower() or "freestyle" in device_type.lower():
        return VirtualCGM(device_id=device_id or f"VIRTUAL_CGM_{datetime.now().timestamp()}", patient=patient)
    elif "pump" in device_type.lower() or "omnipod" in device_type.lower():
        return VirtualInsulinPump(device_id=device_id or f"VIRTUAL_PUMP_{datetime.now().timestamp()}", patient=patient)
    else:
