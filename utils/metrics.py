...
def clarke_error_grid(y_true: np.ndarray, y_pred: np.ndarray) -> Dict[str, float]:
    ...
    for i in range(total):
        ...
        # Zone B - Αποδεκτές
        elif (ref_value >= 180 and pred_value <= 70) or (ref_value <= 70 and pred_value >= 180):
...
def parkes_error_grid(
    y_true: np.ndarray, y_pred: np.ndarray, diabetes_type: str = "type1"
) -> Dict[str, float]:
    """
...
def time_in_range(
    ...
    in_range = np.sum(
        (glucose_values >= target_range[0]) & (glucose_values <= target_range[1])
    )  # 70-180 mg/dL
    in_tight_range = np.sum(
        (glucose_values >= tight_range[0]) & (glucose_values <= tight_range[1])
    )  # 70-140 mg/dL
...
def calculate_metrics(
    y_true: np.ndarray, y_pred: np.ndarray, metrics: Optional[List[str]] = None, **kwargs
