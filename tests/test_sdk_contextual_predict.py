...
def test_contextual_predict_smoke(sdk_with_agent, sample_glucose_window):
    ...
    prediction_obj = sdk_with_agent.contextual_predict(
        glucose_history_window=sample_glucose_window, horizon_minutes=30
    )
...
def test_contextual_predict_agent_finds_pattern(sdk_with_agent, sample_glucose_window):
    ...
    sdk_with_agent.agent.process_glucose_window(
        sample_glucose_window,
        timestamp=datetime.now(),
        extra_metadata={"event_type": "test_pattern"},
    )
...
def test_contextual_predict_agent_disabled(sample_glucose_window):
    ...
    has_agent_context_message = any(
        "Context:" in alert for alert in (contextual_pred.risk_alerts or [])
    )
    ...
    assert (
        contextual_pred.timestamp is not None
    )  # Timestamps will be different, just check existence
...
def test_contextual_predict_with_pump_context(sdk_with_agent, sample_glucose_window):
    ...
    pump_data = PumpContext(
        timestamp=datetime.now(), bolus_amount=5.0, bolus_type="normal", active_basal_rate=0.75
    )

    prediction_obj = sdk_with_agent.contextual_predict(
        glucose_history_window=sample_glucose_window, pump_context=pump_data, horizon_minutes=30
    )
    ...
    for alert in prediction_obj.risk_alerts:
        if "PumpContext provided" in alert and "Bolus 5.0U" in alert:
            ...
            break
    assert (
        found_pump_context_message
    ), "PumpContext acknowledgment message missing or incorrect in risk_alerts"
