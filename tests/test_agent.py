...
def mock_memory_store():
    ...
    dummy_distance = 0.1
    store.search_similar.return_value = [(dummy_search_result_embedding, dummy_metadata, dummy_distance)]
    return store
...
def test_process_glucose_window_list_input(cognitive_agent, mock_encoder, mock_memory_store):
    ...
    result = cognitive_agent.process_glucose_window(glucose_list, timestamp=ts, extra_metadata=extra_meta)
    
    mock_encoder.encode.assert_called_once_with(glucose_list)
    
    expected_metadata = {"source": "agent_processed", "timestamp": ts.isoformat(), "user_id": "test_user"}
    ...
    mock_memory_store.add_embedding.assert_called_once_with(mock_encoder.encode.return_value, expected_metadata)
...
def test_find_similar_patterns(cognitive_agent, mock_encoder, mock_memory_store):
    ...
    mock_memory_store.search_similar.assert_called_once_with(mock_encoder.encode.return_value, k=k_neighbors)
