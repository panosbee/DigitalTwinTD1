...
class T1DDataPreprocessor:
    ...
    def fit_transform(self, 
                     data: pd.DataFrame,
                     target_column: str = 'cgm') -> pd.DataFrame:
        """
    ...
    def create_features(self, 
        ...
        if target_column in data.columns:
            ...
            for window in windows:
                ...
                data[f'{target_column}_slope_{window}'] = self._calculate_slope(data[target_column], window)
    ...
    def normalize_features(self, 
        ...
        for col in cols_to_scale:
            ...
            if col in self.scalers:
                ...
                if valid_mask.any():
                    data.loc[valid_mask, col] = self.scalers[col].transform(
                        data.loc[valid_mask, col].values.reshape(-1, 1)
                    ).flatten()
    ...
    def _time_since_event(self, event_mask: pd.Series) -> pd.Series:
        ...
        for timestamp, is_event in event_mask.items():
            if is_event:
                ...
                time_since[timestamp] = (timestamp - last_event_time).total_seconds() / 60  # σε λεπτά
            else:
    ...
    def inverse_transform_target(self, values: np.ndarray, target_column: str = 'cgm') -> np.ndarray:
...
def validate_data_quality(data: pd.DataFrame, 
                         target_column: str = 'cgm',
                         min_data_points: int = 100) -> Dict[str, Union[bool, float, str]]:
    """
    ...
    if target_column in data.columns:
        ...
        if results['target_coverage'] > 0:
            ...
            iqr = q3 - q1
            outliers = ((data[target_column] < q1 - 1.5 * iqr) | 
                       (data[target_column] > q3 + 1.5 * iqr)).sum()
    ...
    if not results['sufficient_data']:
        results['recommendations'].append(f"Λίγα δεδομένα - χρειάζονται τουλάχιστον {min_data_points}")
