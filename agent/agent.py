...
class PumpContext:
    ...
    programmed_bolus_amount: Optional[float] = (
        None  # Units, if different from delivered (e.g. max bolus)
    )
    ...
    temp_basal_rate_value: Optional[float] = (
        None  # The rate (U/hr) or percentage (e.g., 150 for 150%)
    )
...
class CognitiveAgent:
    ...
    def find_similar_patterns(
        self, glucose_window: Union[List[float], np.ndarray], k: int = 3
    ) -> List[Dict]:
        """
            ...
            formatted_results.append(
                {
                    ...
                    "metadata": meta,
                    "similarity_score": 1
                    - dist,  # Convert distance to similarity (cosine: 1 is identical)
                }
            )
        return formatted_results

    def train_agent_components(
        self, training_data_loader=None, encoder_epochs=5
    ):  # pragma: no cover
        """
...
if __name__ == "__main__":  # pragma: no cover
    ...
    result1 = agent.process_glucose_window(
        window1, timestamp=ts1, extra_metadata={"event": "pre_meal"}
    )
    result2 = agent.process_glucose_window(
        window2, timestamp=ts2, extra_metadata={"event": "post_meal_high"}
    )
    result3 = agent.process_glucose_window(
        window3, timestamp=ts3, extra_metadata={"event": "stable_recovery"}
    )
    ...
    query_window = [
        ...
        88.0,
        85.0,
    ]  # New window, similar to window1/3
    print(f"\nSearching for patterns similar to: {query_window}")
    similar = agent.find_similar_patterns(query_window, k=2)

    for item in similar:
        print(
            f"  Found similar pattern (Timestamp: {item['metadata'].get('timestamp')}, Event: {item['metadata'].get('event')})"
        )
