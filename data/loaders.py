...
def load_data(source: Union[str, Dict], 
              data_type: str = 'auto',
              **kwargs) -> pd.DataFrame:
    """
...
def load_cgm_data(filepath: str, 
                  cgm_column: str = 'glucose',
                  **kwargs) -> pd.DataFrame:
    """
    ...
    glucose_cols = [col for col in df.columns 
...
def load_meal_data(filepath: str,
                   carbs_column: str = 'carbs',
                   **kwargs) -> pd.DataFrame:
    """
    ...
    carb_cols = [col for col in df.columns 
...
def load_insulin_data(filepath: str,
                      basal_column: str = 'basal',
                      bolus_column: str = 'bolus',
                      **kwargs) -> pd.DataFrame:
    """
    ...
    basal_cols = [col for col in df.columns 
                 if any(word in col.lower() for word in ['basal', 'base'])]
    bolus_cols = [col for col in df.columns 
                 if any(word in col.lower() for word in ['bolus', 'meal'])]
...
def load_activity_data(filepath: str,
...
def combine_data_sources(*dataframes: pd.DataFrame, 
                        method: str = 'outer',
