...
try:
    ...
    from intelligence.real_time import (
        RealTimeIntelligenceEngine,
        GlucoseAlert,
        DigitalBiomarkerEngine,
        ClinicalDecisionSupport,
    )
    from optimization.personalized_engine import (
...
class AdvancedDigitalTwinShowcase:
    ...
    def _generate_comprehensive_data(self) -> pd.DataFrame:
        ...
        timestamps = pd.date_range(
            start=start_date, periods=90 * 288, freq="5min"
        )  # 5-minute intervals
        ...
        dawn_effect = np.where((hours >= 4) & (hours <= 8), 20 * np.sin(np.pi * (hours - 4) / 4), 0)
        ...
        for day in range(90):
            # Breakfast (7-9 AM)
            breakfast_idx = np.where(
                (timestamps.day == start_date.day + day)
                & (timestamps.hour >= 7)
                & (timestamps.hour <= 9)
            )[0]
            if len(breakfast_idx) > 0:
                spike_start = np.random.choice(breakfast_idx)
                meal_spikes[spike_start : spike_start + 24] += 80 * np.exp(
                    -np.arange(24) / 12
                )  # 2-hour spike
        ...
        # Stress levels
        stress_levels = (
            3 + 2 * np.sin(2 * np.pi * (hours - 14) / 24) + np.random.normal(0, 1, n_points)
        )
        stress_levels = np.clip(stress_levels, 1, 10)

        # Environmental factors
        temperature = (
            22 + 8 * np.sin(2 * np.pi * (hours - 14) / 24) + np.random.normal(0, 2, n_points)
        )
    ...
    async def _demo_advanced_models(self):
        ...
        try:
            ...
            X = (
                self.synthetic_data[["cgm", "heart_rate", "steps", "stress_level"]]
                .iloc[:-12]
                .values
            )
            ...
            print(
                f"   ✅ Mamba RMSE: {np.sqrt(np.mean((mamba_predictions - y[-100:])**2)):.2f} mg/dL"
            )
            ...
            print(
                f"   ✅ Neural ODE RMSE: {np.sqrt(np.mean((ode_predictions - y[-100:])**2)):.2f} mg/dL"
            )
            ...
            print(
                f"   ✅ Transformer RMSE: {np.sqrt(np.mean((transformer_predictions - y[-100:])**2)):.2f} mg/dL"
            )
    ...
    async def _demo_standard_models(self):
        ...
        print(
            f"✅ Transformer RMSE: {np.sqrt(np.mean((transformer_predictions - y.iloc[-100:])**2)):.2f} mg/dL"
        )
    ...
    async def _demo_real_time_intelligence(self):
        ...
        try:
            ...
            for biomarker in biomarkers[:3]:
                print(
                    f"     • {biomarker.name}: {biomarker.value:.2f} ({biomarker.clinical_significance})"
                )
    ...
    async def _demo_personalized_optimization(self):
        ...
        try:
            ...
            historical_data["insulin_dose"] = (
                historical_data["insulin_bolus"] + historical_data["insulin_basal"] / 12
            )
            ...
            if recommendations.get("consensus"):
                ...
                print(
                    f"   • Predicted TIR Improvement: {consensus.predicted_tir_improvement:+.1f}%"
                )
    ...
    async def _demo_biomarker_discovery(self):
        ...
        try:
            ...
            for day in range(1, 91):
                day_data = (
                    glucose_data[morning_hours == day]
                    if len(glucose_data[morning_hours == day]) > 0
                    else glucose_data[:288]
                )
                if len(day_data) > 10:
    ...
    async def _demo_clinical_decision_support(self):
        ...
        try:
            ...
            print(
                f"   • Best glucose control: {days[best_day]} (avg: {weekly_avg[best_day]:.0f} mg/dL)"
            )
            print(
                f"   • Needs attention: {days[worst_day]} (avg: {weekly_avg[worst_day]:.0f} mg/dL)"
            )
    ...
    async def _demo_causal_inference(self):
        ...
        try:
            ...
            causal_data["treatment"] = (
                causal_data["insulin_bolus"] + causal_data["insulin_basal"] / 12
            )
            ...
            stress_effect_diff = causal_data[high_stress_mask]["treatment"].corr(
                causal_data[high_stress_mask]["outcome"]
            ) - causal_data[~high_stress_mask]["treatment"].corr(
                causal_data[~high_stress_mask]["outcome"]
            )
            ...
            activity_effect_diff = causal_data[active_mask]["treatment"].corr(
                causal_data[active_mask]["outcome"]
    ...
    async def _demo_digital_clinical_trial(self):
        ...
        try:
            ...
            print(
                f"   • Regulatory pathway: {'Pivotal trial ready' if effect_size < -0.5 else 'Additional studies needed'}"
            )
    ...
    async def _demo_explainable_ai(self):
        ...
        try:
            ...
            for feature, contribution in sorted_contributions[:5]:
                ...
                magnitude = (
                    "strongly"
                    if abs(contribution) > 10
                    else "moderately" if abs(contribution) > 5 else "slightly"
                )

                print(
                    f"   • {feature.replace('_', ' ').title()}: {magnitude} {direction} prediction by {abs(contribution):.1f} mg/dL"
                )
            ...
            for i, (feature, contribution) in enumerate(top_features, 1):
                ...
                if feature == "current_glucose":
                    ...
                    explanation += (
                        f"   {i}. Recent carb intake of {current_value:.0f}g is affecting glucose\n"
                    )
                elif feature == "insulin_on_board":
                    explanation += (
                        f"   {i}. Active insulin of {current_value:.1f}U is lowering glucose\n"
                    )
                else:
            ...
            print(
                f"   (95% confidence: {predicted_glucose-prediction_interval:.0f} - {predicted_glucose+prediction_interval:.0f} mg/dL)"
            )
    ...
    async def _create_master_dashboard(self):
        ...
        try:
            ...
            fig.suptitle(
                "🏆 Advanced Digital Twin Library - Master Dashboard",
                fontsize=24,
                fontweight="bold",
                y=0.98,
            )
            ...
            metrics_data = {
                "Time in Range": ((recent_data["cgm"] >= 70) & (recent_data["cgm"] <= 180)).mean()
                * 100,
            ...
            ax11_tir = ax11.twinx()
            bars2 = ax11_tir.bar(
                x + width / 2, tir_results, width, label="Time in Range (%)", color="lightcoral"
            )
