...
class GlucoseEncoder(nn.Module):
    ...
    def encode(self, glucose_window: Union[List[float], np.ndarray]) -> np.ndarray:
        ...
        if glucose_window.ndim == 1:
            ...
            glucose_tensor = (
                torch.tensor(glucose_window, dtype=torch.float32).unsqueeze(0).unsqueeze(-1)
            )
        elif (
            glucose_window.ndim == 2 and glucose_window.shape[0] == 1
        ):  # Already (1, sequence_length)
            ...
            raise ValueError(
                "glucose_window must be a 1D array or a 2D array of shape (1, seq_len) or (seq_len, 1)."
            )
