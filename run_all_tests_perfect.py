...
class PerfectTester:
        ...
        try:
            ...
            try:
                ...
                if "cgm" in synthetic_data.columns and len(synthetic_data) > 0:
                    self.log(
                        f"Data validation passed: {synthetic_data.columns.tolist()}", "SUCCESS"
                    )
    ...
    def test_utils(self):
        ...
        utils_modules = ["utils.data_processing", "utils.evaluation", "utils.visualization"]
    ...
    def print_perfect_summary(self):
        ...
        self.log(
            f"⚠️  Warnings: {warnings}/{total} ({warnings/total*100:.1f}%)",
            "WARNING" if warnings > 0 else "INFO",
        )
