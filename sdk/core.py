...
class DigitalTwinSDK:
    ...
    def __init__(
        ...
        if self.use_agent:
            try:
                ...
                memory_params = self.agent_config.get(
                    "memory_store_params",
                    {"embedding_dim": encoder_params.get("embedding_dim", 64)},
                )
                ...
                final_embedding_dim = encoder_params.get(
                    "embedding_dim", memory_params.get("embedding_dim", 64)
                )
                ...
                print(
                    f"⚠️ Failed to initialize CognitiveAgent: {e}. Agent features will be unavailable."
                )
                print(
                    "   Please ensure agent dependencies are installed (e.g., pip install digital-twin-t1d[cognitive])."
                )
        ...
        print(
            f"🎯 Digital Twin SDK initialized in {mode} mode {'with Cognitive Agent' if self.use_agent else ''}"
        )

    # ===== DEVICE INTEGRATION =====

    def connect_device(
        self, device_type: str, device_id: str, connection_params: Optional[Dict] = None
    ) -> bool:
        """
        ...
        try:
            ...
            print(
                f"✅ Connected to {device_type} (ID: {device_id})"
            )  # Reverted message for consistency
            return True
    ...
    def predict_glucose(
        self, horizon_minutes: int = 30, include_confidence: bool = True, include_risks: bool = True
    ) -> Prediction:
        """
        ...
        if include_confidence:
            predictions, intervals = self.twin.predict(
                latest_data, horizon=horizon_minutes, return_confidence=True
            )
        else:
    ...
    def contextual_predict(
        ...
        if pump_context:
            contextual_info.append(
                f"PumpContext provided: Bolus {pump_context.bolus_amount}U, Basal {pump_context.active_basal_rate}U/hr (Note: Pump context not fully utilized yet)."
            )
    ...
    def get_recommendations(
        self, context: Optional[Dict] = None  # Changed from context: Dict = None
    ) -> Dict[str, Any]:
        """
    ...
    def generate_clinical_report(self, patient_id: str, period_days: int = 30) -> Dict:
        """
    ...
    def get_mobile_widget_data(self) -> Dict:
        ...
        return {
            ...
            "quick_stats": {"time_in_range_today": 0, "last_meal_impact": 0, "insulin_on_board": 0},
        }
    ...
    def export_audit_log(self, start_date: datetime, end_date: datetime) -> pd.DataFrame:
        """
    ...
    def _get_latest_data(self) -> pd.DataFrame:
        ...
        if not self.connected_devices:
            ...
            raise ValueError(
                "No device connected or no data source available to fetch latest data."
            )
        ...
        timestamps = pd.date_range(
            end=datetime.now() - timedelta(minutes=(num_samples - 1) * 5),
            periods=num_samples,
            freq="5min",
        )
        ...
        return pd.DataFrame(
            ...
            "timestamp"
        )  # Set timestamp as index, as expected by some models
    ...
    def _get_insulin_recommendation(self, context: Optional[Dict] = None) -> Dict:
        ...
        return {"bolus": 0, "correction": 0, "confidence": 0.95}
    ...
    def _auto_train_model(self):
        ...
        self.twin.fit(
            data=train_data, target_column="cgm", feature_columns=["insulin", "carbs", "activity"]
        )
...
def quick_predict(glucose_history: List[float], horizon_minutes: int = 30) -> List[float]:
    """
...
def assess_glucose_risk(current_glucose: float, trend: str = "stable") -> Dict[str, Any]:
    """
    ...
    """
    risk = {"level": "normal", "score": 0.0, "actions": []}
