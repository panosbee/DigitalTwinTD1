...
class DiabetesDatasets:
    ...
    def load_synthetic_cgm(
        self,
        n_patients: int = 10,
        days: int = 30,
        sampling_rate: str = "5min",
        include_pump_data: bool = False,
    ) -> pd.DataFrame:  # Phase C: Add pump data flag
        """
        ...
        for patient_id in range(n_patients):
            ...
            for day in range(days):
                for meal_hour in [8, 13, 19]:  # Breakfast, lunch, dinner
                    ...
                    if meal_idx < len(timestamps):
                        ...
                        meal_effect = 60 * np.exp(
                            -((np.arange(len(timestamps)) - meal_peak) ** 2) / (2 * 20**2)
                        )
            ...
            if include_pump_data:
                ...
                patient_df_data["basal_rate"] = np.full(
                    len(timestamps), np.random.uniform(0.5, 1.5)
                )  # U/hr
                ...
                for day_idx in range(days):
                    for meal_hour_offset in [
                        8 * 12,
                        13 * 12,
                        19 * 12,
                    ]:  # 8am, 1pm, 7pm in 5-min intervals
                        meal_event_idx = day_idx * 288 + meal_hour_offset
                        if meal_event_idx < len(timestamps):
                            # Carbs at meal time
                            patient_df_data["carbs_input"][meal_event_idx] = np.random.uniform(
                                30, 80
                            )
                            # Bolus slightly before or at meal time
                            bolus_idx = meal_event_idx - np.random.randint(
                                0, 3
                            )  # 0-10 mins before meal
                            if bolus_idx >= 0:
                                patient_df_data["bolus_amount"][bolus_idx] = patient_df_data[
                                    "carbs_input"
                                ][meal_event_idx] / np.random.uniform(
                                    10, 15
                                )  # Simple ICR
...
if __name__ == "__main__":
    ...
    data = load_diabetes_data(
        "synthetic", n_patients=3, days=7
    )  # Export για το testing script - αυτό χρειαζόταν!DatasetManager = DiabetesDatasets  # Alias για backward compatibility
