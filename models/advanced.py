...
class MambaModel(BaseModel):
    ...
    def predict(self, X: pd.DataFrame, horizon: int = 30, return_uncertainty: bool = False, **kwargs) -> np.ndarray:
...
class MultiModalModel(BaseModel):
    ...
    def _build_model(self, input_dims: Dict[str, int]):
        ...
        class MultiModalNetwork(nn.Module):
            def __init__(self, input_dims, cgm_dim, wearable_dim, lifestyle_dim, 
            ...
            def forward(self, cgm_data, wearable_data, lifestyle_data):
                ...
                attended, self.attention_weights = self.cross_attention(
                    fused, fused, fused
                )
        ...
        return MultiModalNetwork(
            input_dims, self.cgm_encoder_dim, self.wearable_encoder_dim,
            self.lifestyle_encoder_dim, self.fusion_dim, self.num_heads, self.dropout
        )
    
    def fit(self, X: pd.DataFrame, y: pd.Series, 
            modality_columns: Dict[str, List[str]], **kwargs) -> 'MultiModalModel':
        """
